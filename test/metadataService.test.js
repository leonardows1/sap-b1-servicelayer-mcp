...
const METADATA_RESPONSE = { status: 200, body: null, text: XML, setCookies: [] };

function makeFakeClient() {
  return createFakePort({
    responses: {
      "GET /$metadata": METADATA_RESPONSE,
      ...
      "POST /OrdersService_Cancel": { status: 204, body: null, text: "", setCookies: [] },
      "POST /BrokenAction": {
        status: 500,
...
test("entityExists: true/false según el entity set", async () => {
  const svc = createMetadataService(makeFakeClient());
  assert.equal(await svc.entityExists("BusinessPartners"), true);
  assert.equal(await svc.entityExists("ItemStock"), false);
});
